...
## Terminology: what is meant by "seam"

Throughout this document a **seam** is a junction point between two protocol
components where each side has its own trust assumption and nothing enforces
that they agree. Concretely: protocol A writes something to a shared store
("trusted under A's model"); protocol B later reads from that store and acts
on it ("assumed to satisfy B's, possibly stronger, model"). When A's model is
weaker than B's and the disagreement is not detected, an attacker who passes
A's weaker check gets B's stronger privileges. Each of the seams catalogued
below has this shape.

The word is borrowed from Michael Feathers, *Working Effectively with Legacy
Code*: "A seam is a place where you can alter behavior in your program without
editing in that place." In Feathers' usage the seam is a substitutability
point exploited constructively for testing; here it is the same structural
boundary exploited destructively by an attacker who substitutes their own
state.

The paper *Signal Lost (Integrity)* itself doesn't use "seam" — it talks about
"integration points" and "cross-protocol composition". We picked "seam"
because it captures the sense of "a gap that looks like a continuous surface."
